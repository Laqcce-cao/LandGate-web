...
const features: Array<{
...
}> = [
  {
    ...
    tone: 'bg-[#F1F2F1] text-[#1E2A44] ring-[#D9DCD7]',
  },
  {
    icon: 'key',
    title: '密钥与权限边界',
    description: '为团队、项目和环境创建独立 API Key，调用记录清晰，权限边界明确。',
    tone: 'bg-[#FAF8F2] text-[#A77A45] ring-[#D9C8AF]',
  },
  {
    icon: 'creditCard',
    title: '余额与模型价格',
    description: '模型价格、充值扣费、余额流水和 Token 消耗统一统计，成本变化可追踪。',
    tone: 'bg-[#F0F3F1] text-[#65776B] ring-[#D7DED8]',
  },
];
...
function GatewayRoutePreview() {
  return (
    <div className="relative w-full max-w-[430px]">
      <div className="absolute -inset-4 rounded-[24px] bg-[#A77A45]/[0.08] blur-2xl dark:bg-[#A77A45]/[0.12]" />
      <div className="relative overflow-hidden rounded-2xl border border-[#D8D5CC] bg-white/90 p-5 shadow-[0_28px_80px_-42px_rgba(16,20,24,0.58)] backdrop-blur-2xl dark:border-white/10 dark:bg-[#0E1116]/72">
        <div className="absolute inset-x-5 top-0 h-px bg-[#A77A45]/45" />
        <div className="relative mb-5 flex items-center justify-between">
          <div>
            <p className="text-xs font-medium uppercase tracking-[0.18em] text-[#8A6235] dark:text-[#D8BE96]">Gateway Route</p>
            <h2 className="mt-1 text-xl font-semibold text-slate-950 dark:text-white">LandGate 网关路由</h2>
          </div>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#D7DED8] bg-[#F0F3F1] px-3 py-1 text-xs font-semibold text-[#52665B] dark:border-white/10 dark:bg-white/[0.06] dark:text-[#AFC2B5]">
            <span className="h-1.5 w-1.5 rounded-full bg-[#65776B]" />
            Online
          </span>
        </div>

        <div className="relative rounded-xl border border-[#E0DED7] bg-[#F7F6F2]/90 p-4 dark:border-white/10 dark:bg-white/[0.045]">
          <div className="mb-4 flex items-center justify-between gap-3">
            ...
            <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[#FAF8F2] text-[#A77A45] ring-1 ring-[#D9C8AF] dark:bg-white/[0.06] dark:text-[#D8BE96] dark:ring-white/10">
              <Icon name="key" size="md" />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3">
            <div className="rounded-xl bg-white/82 p-3 ring-1 ring-slate-200/70 dark:bg-slate-900/62 dark:ring-white/10">
        ...
        <div className="my-5 flex items-center gap-3 px-1">
          {['Client', 'LandGate', 'Upstream'].map((item, index) => (
            <div key={item} className="flex flex-1 items-center gap-3">
              ...
              {index < 2 && <Icon name="chevronRight" size="sm" className="shrink-0 text-[#A77A45]/80" />}
            </div>
          ))}
        </div>

        <div className="relative rounded-xl border border-[#D9C8AF] bg-[#FAF8F2] p-4 dark:border-[#D8BE96]/15 dark:bg-[#A77A45]/[0.08]">
          <div className="flex items-center justify-between gap-4">
            ...
            <span className="shrink-0 rounded-full bg-white px-3 py-1 text-xs font-semibold text-[#6F512D] ring-1 ring-[#D9C8AF] dark:bg-white/10 dark:text-[#D8BE96] dark:ring-white/10">
              42 ms
            </span>
          </div>
        </div>
...
export default function HomePage() {
  ...
  return (
    <main className="relative flex min-h-screen flex-col overflow-x-hidden bg-[#F6F6F3] text-slate-950 dark:bg-dark-950 dark:text-white">
      <AmbientScene />

      <header className="fixed inset-x-0 top-0 z-50 border-b border-[#101418]/10 bg-[#FBFBFA]/72 px-6 py-3 backdrop-blur-2xl dark:border-white/10 dark:bg-dark-950/52">
        <nav className="mx-auto flex max-w-6xl items-center justify-between">
          <Link to="/" className="flex items-center gap-3">
            <span className="relative flex h-10 w-10 items-center justify-center rounded-lg bg-[#101418] text-sm font-black text-white shadow-md shadow-black/10 dark:bg-[#E8E2D8] dark:text-[#101418]">
              <span className="absolute inset-[3px] rounded-md border border-[#A77A45]/45" />
          ...
          <div className="flex items-center gap-3">
            ...
            <Link
              to={primaryPath}
              className="inline-flex items-center rounded-lg bg-slate-900 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-slate-800 dark:bg-white dark:text-slate-950"
            >
              {token ? '控制台' : '开始使用'}
            </Link>
          </div>
        ...
        <div className="mx-auto max-w-6xl">
          <section className="mb-12 flex flex-col items-center justify-between gap-12 lg:flex-row lg:gap-16">
            <div className="flex-1 text-center lg:text-left">
              <h1 className="mb-5 text-4xl font-semibold text-[#101418] dark:text-white md:text-5xl lg:text-6xl">
                LandGate
              </h1>
              <p className="mb-5 max-w-2xl text-lg font-medium leading-8 text-[#2A3036] dark:text-dark-200 md:text-xl">
              ...
              <Link to={primaryPath} className="btn btn-primary px-8 py-3 text-base">
                {token ? '进入控制台' : '立即开始'}
          ...
          <section className="mb-12 flex flex-wrap items-center justify-center gap-4 md:gap-6">
            {featureTags.map((tag) => (
              ...
              >
                <Icon name={tag.icon} size="sm" className="text-[#A77A45]" />
          ...
          <section id="features" className="mb-12 grid gap-6 md:grid-cols-3">
            {features.map((feature) => (
              <article
                key={feature.title}
                className="group rounded-xl border border-[#D8D5CC] bg-white/82 p-6 backdrop-blur-sm transition-all duration-300 hover:border-[#D9C8AF] hover:shadow-xl hover:shadow-black/[0.055] dark:border-white/10 dark:bg-white/[0.055]"
              >
                <div className={`mb-4 flex h-12 w-12 items-center justify-center rounded-xl ring-1 transition-transform group-hover:scale-105 ${feature.tone}`}>
